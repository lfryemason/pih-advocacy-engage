...
for (const theme of themes) {
  test.describe(`login page (${theme})`, () => {
    ...
    test('renders branding and form', async ({ page }) => {
      await expect(page.getByRole('heading', { level: 1 })).toContainText('Advocacy Tracking');
    ...
    test('links to sign up and forgot password', async ({ page }) => {
      await expect(page.getByRole('link', { name: 'Sign up' })).toHaveAttribute('href', '/auth/sign-up');
  ...
  test.describe(`sign up page (${theme})`, () => {
    ...
    test('links back to login', async ({ page }) => {
      await expect(page.getByRole('link', { name: 'Login' })).toHaveAttribute('href', '/auth/login');
    });
