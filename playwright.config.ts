  ...
  testIgnore:
    process.env.TEST_SUITE === "regression"
      ...
      : process.env.TEST_SUITE === "e2e" || !process.env.CI
        ? "**/regression/**"
        : undefined,
  /* Run test files sequentially — tests share a single Supabase DB so parallel
     workers cause race conditions on beforeEach resets. */
  fullyParallel: false,
  /* Fail the build on CI if you accidentally left test.only in the source code. */
  forbidOnly: !!process.env.CI,
  /* Retry on CI only */
  retries: process.env.CI ? 2 : 0,
  /* Single worker — prevents cross-file DB interference. */
  workers: 1,
