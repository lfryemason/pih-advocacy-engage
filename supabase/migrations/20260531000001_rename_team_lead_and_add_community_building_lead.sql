...
update public.team_memberships set role = 'team_coordinator' where role = 'team_lead';

alter table public.team_memberships
  add constraint team_memberships_role_check
  check (role in ('member', 'team_coordinator', 'fundraising_lead', 'advocacy_lead', 'coach', 'community_building_lead'));
...
as $$
begin
  if p_new_role not in ('member', 'team_coordinator', 'fundraising_lead', 'advocacy_lead', 'coach', 'community_building_lead') then
